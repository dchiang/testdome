...
public class DecoratorStream extends OutputStream
{
    ...
    public void write(byte[] b, int off, int len) throws IOException {
        ByteArrayOutputStream baos = (ByteArrayOutputStream) this.stream;
        String currentOutput = baos.toString("UTF-8");
        if(!currentOutput.startsWith(this.prefix)){
          byte[] prefixArray = this.prefix.getBytes("UTF-8");
          baos.write(prefixArray,0,prefixArray.length);
        }
